    ...
    if ext.lower() in video_exts:
        ...
        if subtitle_path:
            ...
            def run_ffmpeg_with_progress(video, sub, out_file):
                ...
                subtitle_size = get_file_size_mb(subtitle_path)
                ...
                try:
                    ...
                    for line in process.stderr:
                        if "time=" in line:
                            ...
                            if match:
                                ...
                                size_percent = (output_size / input_total_size) * 100 if input_total_size > 0 else 0
                                draw_bar(percent, video, sub, out_file, start_time, output_size, size_percent)
                    process.wait()
                    if process.returncode == 0:
                        final_output_size = get_file_size_mb(output_path)
                        print(f"\n Output size: {final_output_size:.2f} MB")
                        print(f" Original (video + sub): {input_total_size:.2f} MB")
                        if final_output_size > input_total_size:
                            print(f" File grew by {(final_output_size - input_total_size):.2f} MB")
                        else:
                            print(f" File shrunk by {(input_total_size - final_output_size):.2f} MB")
                    ...
                    print(f"\n Error running ffmpeg: {e}\n")
                            ...
                            # ---PROGRESS BAR--- #
            def draw_bar(percent, video, sub, out_file, start_time, output_size, size_percent):
                ...
                sys.stdout.write("\r")
                sys.stdout.write(
                    f"\rProcessing: {video} + {sub} |{bar}| {i:3.0f}% "
                    f"E.T.A: {eta} | Out: {output_size:.1f}MB ({size_percent:.1f}%)"
                )
                sys.stdout.flush()
